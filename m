...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 4BED09CC9F
	for <lists+linux-iio@lfdr.de>; Mon, 26 Aug 2019 11:34:27 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726953AbfHZJe0 (ORCPT <rfc822;lists+linux-iio@lfdr.de>);
        Mon, 26 Aug 2019 05:34:26 -0400
Received: from mail.kernel.org ([198.145.29.99]:59220 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726616AbfHZJe0 (ORCPT <rfc822;linux-iio@vger.kernel.org>);
        Mon, 26 Aug 2019 05:34:26 -0400
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 84AED22CED;
        Mon, 26 Aug 2019 09:34:21 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1566812062;
        bh=LIIUwvsHQOia1t7q0n6ZjP+xT/RoFugzwtt9wiiZ0jA=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=B+cuP3gZ1raphCACGbSiGeN3lH5hlyeg25pnb8cHG7weAhdnOzMj9GTC4bfMjqLaW
         hycusLmT3uODbVkbWPTXxcI1WPxUbCIgyiRI2e7Z0cKHVjibeHKuwfLYN8jOB6Ntaa
         Y3Q1r6wjOt6dMiXLoyNgyx2SIXW3orrcQNoSSl9U=
Date:   Mon, 26 Aug 2019 10:34:18 +0100
...
Subject: Re: [PATCH] staging:iio:meter add driver for ADE9000
Message-ID: <20190826103418.3d143742@archlinux>
In-Reply-To: <20190814082037.5214-1-leonardp@tr-host.de>
References: <20190814082037.5214-1-leonardp@tr-host.de>
...
On Wed, 14 Aug 2019 10:20:37 +0200
...
+CC John Syne who is normally interested in these parts.

I'll take a look at this driver first, then catch up with your earlier
email.  May lead to less than coherent responses from me, but have
to start somewhere.

We've been round a few loops in the past with meter drivers but
never quite gotten them out of staging.  As you've realised
they push the boundaries of the IIO interfaces in all sorts of
ways.  Most recent version of this discussion was probably
in this thread.

https://lore.kernel.org/linux-iio/3af0e5a795059a8418cf08ff29f05f8d5e00da9b.1520382753.git.rodrigosiqueiramelo@gmail.com/
particularly
https://lore.kernel.org/linux-iio/F6FE75D3-5289-4675-9345-F46CABEFA99E@gmail.com/ and onwards

I would read that discussion first before diving too much into the ABI
questions.  From what I recall our conclusion was that a few core
changes would be needed to give the depth of description needed for these
devices.

Clearly you are proposing this for staging.  Why?
Staging is useful for various reasons, and it's good to understand
why someone wants to put a particular driver in there.  Note the
meter drivers there all predate moving any of the IIO subsystem
out of staging. (also note there were more drivers that have been
dropped due to parts not being available anymore!)

For now I'm just going to review this as a new submission.

A few useful things for v2:

* Docs for sysfs ABI.  It is much easier to discuss ABI when we have
  both code and documentation.  Normally goes under
  Documentation/ABI/testing/sysfs-bus-iio-*
* For complex devices, it's useful to just have a listing of what
  is created under sysfs.

There are abstractions in here that don't make sense for this part.
I think it only supports SPI, so no need to abstract reads and writes
for example.

For now, there is a lot still to do in here.  I may well have commented
on things you are intending to clean up anyway!
...
return ade900... Though I would assume some checking of those inputs
would be wise.

Can you actually write all of these?  Seems a bit 'unusual'.
...
Drop comments that are obvious from the code. They are just noise
that may become wrong as the driver evolves.
...
Comment syntax in most of the kernel including IIO is.
/*
 * 1: interrupt
...
 */
...
This abstraction doesn't add anything. Call those functions directly.

> +	st->irq = spi->irq;
> +	st->run = 0;

Assuming run = 0 is an obvious default, no need to set it
as that structure will have been zero initialized anyway.
...
Why have this header at all?  Only put things in a header if they
need to be shared between multiple c files.
...
A basic rule of kernel coding 'style' is to never put anything in a header
that can go in a c file.  
> +
...
This confuses me.  We have a scale for an output voltage
but no means of controlling it?

Ah, you have one of these structures for every element in sysfs.
I guess so you can use the address field to point at the register.

Add a layer of indirection so your _phase is used to index
into an array of structure with the scale + offset +... 
addresses. That way you can have far fewer apparent channels.
...
You shouldn't need to do that.  Setting the name field in the iio_info
struct should do this automatically.

> +static IIO_DEVICE_ATTR(run, 0660,
> +		       ade9000_get_attr, ade9000_set_attr, ADE9000_RUN);

I'm not going to look too closely at these until we have documentation.
This ABI can be hard to get write.
