...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 70AD021D81B
	for <lists+linux-iio@lfdr.de>; Mon, 13 Jul 2020 16:16:16 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729985AbgGMOQP (ORCPT <rfc822;lists+linux-iio@lfdr.de>);
        Mon, 13 Jul 2020 10:16:15 -0400
Received: from mail.kernel.org ([198.145.29.99]:40340 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1729823AbgGMOQP (ORCPT <rfc822;linux-iio@vger.kernel.org>);
        Mon, 13 Jul 2020 10:16:15 -0400
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id DC1382072D;
        Mon, 13 Jul 2020 14:16:12 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1594649773;
        bh=OMNH0LvVOAKV1GW26giOSHQeA/k3WV+Kt4uwD7OhY6g=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=B4UnecAF4Kl51ZaLzNfyGNvv4hKSDX5EEZrLqHZNX3RKzzbAGYlX9bW3754er1K2V
         Xvy7FS/XeQxOraJ6xlprFtr4QhkONKRAvrlDWtNP9VoXE+RQgxy9HaUoFhTo5iLJ29
         ksFiqJp1XnJ9HjrmTdgt6FUDhMOqIcyzSYGgXymM=
Date:   Mon, 13 Jul 2020 15:16:09 +0100
...
Message-ID: <20200713151609.12adb152@archlinux>
In-Reply-To: <01c5ed2a8e79e2ad629622a01836aadd87f01525.camel@analog.com>
References: <20200712145703.526c8b46@archlinux>
        ...
        <01c5ed2a8e79e2ad629622a01836aadd87f01525.camel@analog.com>
...
On Mon, 13 Jul 2020 11:01:38 +0000
"Ardelean, Alexandru" <alexandru.Ardelean@analog.com> wrote:

> On Mon, 2020-07-13 at 11:48 +0100, Jonathan Cameron wrote:
...
> >=20
> > On Mon, 13 Jul 2020 05:15:50 +0000
> > "Ardelean, Alexandru" <alexandru.Ardelean@analog.com> wrote:
> >  =20
> > > On Sun, 2020-07-12 at 14:57 +0100, Jonathan Cameron wrote: =20
> > > > The following changes since commit
> > > > b791d1bdf9212d944d749a5c7ff6febdba241771:
> > > >    =20
> > >=20
> > > I'm wondering if I may have caused some confusion with some re-send,
> > > and
> > > maybe this patch got omitted by accident.
> > > Bt this patch is pretty important to include in the rework series [th=
at
> > > I
> > > did]:
> > > https://urldefense.com/v3/__https://patchwork.kernel.org/patch/116331=
05/__;!!A3Ni8CS0y2Y!qQbthpaBAjQKr2jenpPCf11vtiASc4FqcPXULtYYdF4JT9FHgKu9O9I=
cziYM0969r2x-Pg$=20
> > >=20
> > > Keeping it here can be problematic, as [with the iio_dev_opaque
> > > type/change
> > > series], using the iio_priv_to_dev() helper can be problematic; the
> > > offset
> > > isn't correct anymore and can cause undefined behavior. =20
> >=20
> > I think I just messed up fixing an issue where I didn't have the at91
> > patch
> > that was using this and got a build issue.
> >=20
> > Right now it should be a hypothetical issue as there are no users in
> > tree, but
> > I agree we definitely want to add your patch to make sure we don't cause
> > trouble
> > for any new drivers or out of tree patches.
> >=20
> > I'll apply it on top and send a v2 pull request later today.
> >=20
> > Thanks for taking a close look at this Alexandru! =20
>=20
> In this tree, there's only the 'drivers/iio/adc/ingenic-adc.c' driver usi=
ng
> the helper, so it may cause a build error if we just apply the patch I
> mentioned.
>=20
I've dropped the ingenic patch and added the one I missed originally.
Tree is pushed out to let the autobuilders poke at it.  If that all looks
fine I'll probably do an updated pull request sometime in next few days.
...
> >=20
> >  =20
> > > >   Merge tag 'locking-kcsan-2020-06-11' of
> > > > git://git.kernel.org/pub/scm/linux/kernel/git/tip/tip (2020-06-11
> > > > 18:55:43 -0700)
> > > >=20
...
> > > > 77864b0663b0206ff1e5878e40075310c54528b7:
> > > >=20
> > > >   Replace HTTP links with HTTPS ones: drivers/iio (2020-07-12
> > > > 14:45:36
> > > > +0100)
> > > >=20
> > > > ----------------------------------------------------------------
> > > > First set of new device support, cleanups and features for IIO in t=
he
> > > > 5.9
> > > > cycle
> > > >=20
> > > > Some new devices, but particularly good this time is the core rework
> > > > coming
> > > > from Alexandru. Some of this has been in Analog's tree a long time,
> > > > but
> > > > other
...
> > > > New device support
> > > > * qcom pmic7 adc
> > > >   - New driver using common qcom-vadc library. Some associated
> > > > cleanups
> > > > and
> > > >     refactors.
> > > > * invensense icm42600
> > > >   - New driver supporting icm42600, icm42602, icm42605 and icm42622
> > > > via
> > > > i2c
> > > >     or spi.  These are all 6 axis IMUs with gyro and accelerometers.
> > > >     Driver supports buffered modes using the hardware fifo and
> > > > interpolation
> > > >     for accurate timestamps.
> > > > * sensirion scd30
> > > >   - New driver for this carbon dioxide sensor including i2c and
> > > > serial
> > > >     interfaces + bindings.
> > > >=20
> > > > Features
...
> > > >   - Support SPI.
> > > > * bmc150
> > > >   - Document and add support for bmc156b and bmm150b, tidy up _magn
> > > > endings.
> > > > * bmi160
...
> > > > * Assign parent device in the core rather than every driver. A few
> > > > devices
> > > >   need to provide specific non standard parents, so there is support
> > > > for
> > > >   overriding.
> > > > * Start to take parts of struct iio_dev opaque to the drivers.
> > > >   This will be a long term job, but should reduce the number of
> > > > drivers
> > > >   we get that use parts that are currently only 'internal' by
> > > > documentation.
> > > > * Move attach and detach of pollfunc to the core.  Every triggered
> > > > buffer
> > > >   using driver had to do the same thing, so lets do it in the
> > > > core.  The
> > > >   hard part here was getting all the drivers into canonical form so
> > > > there
> > > >   would be no functional changes in this final patch.  That's taken
> > > > quite
...
> > > > * core
> > > >   - Make iio_device_get_drvdata take a const struct iio_dev *
> > > > avoiding
> > > > some
> > > >     nasty casts.
> > > > * ADCs
> > > >   - Drop lots of users of of_match_ptr macro, includes of
> > > > mod_devicetable.h
> > > >     and CONFIG_OF protections.  These prevent use of ACPI PRP0001
> > > > with
> > > > these
...
> > > > * ad5592r
> > > >   - Constify iio_chan_spec_ext_info
> > > >   - Avoid use of iio_priv_to_dev (precursor to taking parts of
> > > > iio_dev
> > > > opaque)
> > > > * ad8366
...
> > > > opaque)
> > > > * ingenic
> > > >   - Add error checks on clk_neable calls
> > > >   - Add xlate callback to retrieve correct channel index.
> > > > * inv_mpu
...
> > > > opaque)
> > > > * ti-am335x
> > > >   - Use managed allocations where straight forward in probe functio=
n.
> > > > * tsl2563
> > > >   - Avoid use of iio_priv_to_dev (precursor to taking parts of
> > > > iio_dev
...
> > > >       iio: light: tsl2563: pass iio device as i2c_client private da=
ta
...
> > > >       iio: light: lm3533-als: use iio_device_set_parent() to assign
> > > > parent
...
> > > >       iio: core: move channel list & group to private iio device
> > > > object
> > > >       iio: core: move iio_dev's buffer_list to the private iio devi=
ce
> > > > object
...
> > > >=20
> > > > Andy Shevchenko (1):
> > > >       iio: imu: inv_mpu6050: Drop double check for ACPI companion
> > > > device
> > > >=20
> > > > Artur Rojek (3):
> > > >       dt-bindings: iio/adc: Convert ingenic-adc docs to YAML.
> > > >       IIO: Ingenic JZ47xx: Error check clk_enable calls.
> > > >       IIO: Ingenic JZ47xx: Add xlate cb to retrieve correct channel
> > > > idx
> > > >=20
> > > > Christian Oder (1):
...
> > > >=20
> > > > Jonathan Albrieux (9):
> > > >       dt-bindings: iio: magnetometer: ak8975: reword gpios, add
> > > > interrupts, fix style
> > > >       dt-bindings: iio: magnetometer: ak8975: convert format to yam=
l,
> > > > add
> > > > maintainer
> > > >       dt-bindings: iio: magnetometer: ak8975: add gpio reset support
> > > >       iio: magnetometer: ak8975: Fix typo, uniform measurement unit
> > > > style
...
> > > >       iio:adc:max11100: Drop of_match_ptr protection / add
...
> > > >       iio:adc:ti-adc081c: Drop of_match_ptr and change to
> > > > mod_devicetable.h
> > > >       iio:adc:ti-adc0832: drop CONFIG_OF and of_match_ptr protectio=
ns
> > > >       iio:adc:ti-adc084s021: drop of_match_ptr protection
> > > >       iio:adc:ti-adc161s626: Drop of_match_ptr protection.
> > > >       iio:adc:ti-tlc4541: Drop CONFIG_OF and of_match_ptr
> > > > protections.
...
> > > >       dt-bindings: iio: bmc150_magn: Document and fix missing
> > > > compatibles
...
> > > >=20
> > > > Ludovic Desroches (1):
> > > >       MAINTAINERS: adc: at91-sama5d2_adc: remove myself as co-
> > > > maintainer
> > > >=20
> > > > Matt Ranostay (1):
> > > >       iio: chemical: atlas-ezo-sensor: move ret variable scope in
> > > > atlas_ezo_read_raw()
> > > >=20
> > > > Nuno S=C3=A1 (1):
...
> > > >  drivers/iio/adc/ingenic-adc.c                      |  47 +-
...
> > > >  429 files changed, 8004 insertions(+), 1639 deletions(-)
...
> > > >  delete mode 100644
> > > > Documentation/devicetree/bindings/iio/adc/qcom,spmi-
> > > > vadc.txt
> > > >  create mode 100644
> > > > Documentation/devicetree/bindings/iio/adc/qcom,spmi-
> > > > vadc.yaml
...
> > > >  create mode 100644
> > > > Documentation/devicetree/bindings/iio/magnetometer/asahi-
> > > > kasei,ak8975.yaml
...
> > > >  create mode 100644 drivers/iio/imu/inv_icm42600/inv_icm42600_accel=
.c
> > > >  create mode 100644
...
> > > >  create mode 100644 include/linux/iio/iio-opaque.h   =20
> >=20
> >  =20
