...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id F027216833D
	for <lists+linux-iio@lfdr.de>; Fri, 21 Feb 2020 17:26:57 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726747AbgBUQ05 (ORCPT <rfc822;lists+linux-iio@lfdr.de>);
        Fri, 21 Feb 2020 11:26:57 -0500
Received: from mail.kernel.org ([198.145.29.99]:35396 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1725947AbgBUQ04 (ORCPT <rfc822;linux-iio@vger.kernel.org>);
        Fri, 21 Feb 2020 11:26:56 -0500
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 858B020578;
        Fri, 21 Feb 2020 16:26:54 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1582302415;
        bh=qEt2aAdJQy+CyTIKJlJTklP+9kPzqsyzNBeLalHtM2Y=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=R6UacX4yAEnsboWYrgNk/ViOG/flbFaQ8tg8SQJLsIcBay61cgQAfwglCsP82QJux
         g/knT+NVmfKa3YsZ24FvhJj2K7d40fZA4V9W4cK5mpq3YqSpiPYfyZRTCxoz/L6rAP
         jSE5oVvzxVjGXoJ4UpzS9rbYlqG9mlg6Alk1JyFc=
Date:   Fri, 21 Feb 2020 16:26:51 +0000
From:   Jonathan Cameron <jic23@kernel.org>
To:     Jean-Baptiste Maneyrol <jmaneyrol@invensense.com>
Cc:     linux-iio@vger.kernel.org
Subject: Re: [PATCH] iio: pressure: icp10100: add driver for InvenSense
 ICP-101xx
Message-ID: <20200221162651.658e04c6@archlinux>
In-Reply-To: <20200220160550.30497-1-jmaneyrol@invensense.com>
References: <20200220160550.30497-1-jmaneyrol@invensense.com>
...
On Thu, 20 Feb 2020 17:05:50 +0100
...
A few minor things inline but looks pretty good to me.

I would only introduce the flexibility to support multiple devices
when you add the second device and hence can show it is needed.
...
Whenever we have a label like this in a switch block I always think
we are probably better off factoring the code out to a separate
function.

So I would pull the stuff between claim and release direct out...
...
As mentioned below. only add this complexity once it's needed.
For now there is only one chip supported and we know it's name.

> +	if (match) {
> +		chip =3D (enum icp10100_chip)match;
> +		name =3D client->name;
> +	} else if (id) {
> +		chip =3D (enum icp10100_chip)id->driver_data;
> +		name =3D id->name;
> +	} else {
> +		return -ENOSYS;
> +	}
...
Blank line here helps readability in my opinion.
...
> +		.data =3D (void *)ICP10100,
Unless you are going to follow this almost immediately with other device
support, please don't include stuff intended to make it more generic.
For now it only supports this one part so there is no advantage
in having it as a value checked anywhere.
