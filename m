...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 41E7812D357
	for <lists+linux-iio@lfdr.de>; Mon, 30 Dec 2019 19:24:09 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727511AbfL3SYI (ORCPT <rfc822;lists+linux-iio@lfdr.de>);
        Mon, 30 Dec 2019 13:24:08 -0500
Received: from saturn.retrosnub.co.uk ([46.235.226.198]:37410 "EHLO
        saturn.retrosnub.co.uk" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1727403AbfL3SYH (ORCPT
        <rfc822;linux-iio@vger.kernel.org>); Mon, 30 Dec 2019 13:24:07 -0500
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        by saturn.retrosnub.co.uk (Postfix; Retrosnub mail submission) with ESMTPSA id 7D2509E774E;
        Mon, 30 Dec 2019 18:23:39 +0000 (GMT)
Date:   Mon, 30 Dec 2019 18:23:26 +0000
...
Cc:     IIO <linux-iio@vger.kernel.org>,
        ...
        Romain =?UTF-8?B?Um9mZsOp?= <romain.roffe@parrot.com>,
        =?UTF-8?B?UsOpbWkg?= =?UTF-8?B?TGVmw6h2cmU=?= 
        <remi.lefevre@parrot.com>,
...
Message-ID: <20191230182326.47b89c14@archlinux>
In-Reply-To: <1575905163.3.4@crapouillou.net>
...
This separates somewhat the main data flow from the metadata and allows
...
Agreed, this would be useful.  So taking ADCs there are 3 cases I think.

1) Overflow of software buffer (that needs to be in core)
2) Overflow of hardware buffer (that needs to be at least partly driver specific)
3) DMA overflow (descriptor not provided in time).

It's made more fun by the fact these may be per device, or per channel (for
...
report 'unexpected / error cases' in a generic fashion.  However it's never
...
It may be hard to put together a generic way of doing this in kernel. GPUs
and similar get way with this stuff because they have very rich APIs.

If it's a rigid multiplex case, then we can do something using consumer
...
Agreed. That area has moved on somewhat since the IIO code was written!
...
Splice also tends to put annoying limits on size of blocks. Not totally 
...
really needs to be realtime).  If I were designing a platform to do this
...
Compatibility at the kernel interfaces is a must, but we do have a little
